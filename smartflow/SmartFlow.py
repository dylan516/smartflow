...
class SmartFlow(object):
    ...
    def __load_parameters_selector(self):
        ...
        for rowx in range(sheet_parameters.nrows):
            ...
            for param_index in range(2, len(row), 2):
                if (row[param_index]):
                    tm_parameters.append(str(row[param_index]))
                    tm_parameters.append(str(row[param_index + 1]))
    ...
    def __load_timing_selector(self):
        ...
        for rowx in range(sheet_timing.nrows):
            ...
            if (rowx < 2):
                continue
            if (row[0] != ''):
                timing_selector[row[0]] = [cell2str(row[1]), cell2str(row[2]), cell2str(row[3])]
    ...
    def __load_level_selector(self):
        ...
        for rowx in range(sheet_level.nrows):
            ...
            if (rowx < 2):
                continue
            if (row[0] != ''):
                level_selector[row[0]] = [cell2str(row[1]), cell2str(row[2]), cell2str(row[3])]
    ...
    def __load_floweditor(self):
        ...
        for rowx in range(sheet_floweditor.nrows):
            ...
            for index in range(len(row)):
                row_dict[header[index]] = row[index]
            if (('testsuite_name_regex' in row_dict) and \
                    (row_dict['testsuite_name_regex'].strip() != '')):
